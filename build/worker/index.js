...
async function ensureUserContext(request, env) {
    ...
    let membership = await queryFirst(env.DB, 'SELECT role FROM household_members WHERE household_id = ? AND user_id = ?', householdId, user.id);
    if (!membership) {
        const count = await queryFirst(env.DB, 'SELECT COUNT(*) AS count FROM household_members WHERE household_id = ?', householdId);
...
function assertString(value, field, maxLength = 120, required = true) {
    ...
    if (trimmed.length > maxLength)
        throw new HttpError(400, 'VALIDATION_ERROR', `${field}内容太长`, { field, maxLength });
    return trimmed;
}
...
function assertDate(value, field = '日期') {
...
async function audit(env, context, action, entityType, entityId, beforeData, afterData) {
    await run(env.DB, 'INSERT INTO audit_logs (id, household_id, actor_user_id, action, entity_type, entity_id, before_data, after_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', crypto.randomUUID(), context.householdId, context.userId, action, entityType, entityId, beforeData == null ? null : JSON.stringify(beforeData), afterData == null ? null : JSON.stringify(afterData));
}
async function accountExists(env, householdId, accountId) {
...
async function categoryExists(env, householdId, categoryId, type) {
...
function accountBalanceExpression(alias = 'a') {
    return `
...
async function getAccounts(env, context, includeArchived = false) {
    return queryAll(env.DB, `SELECT a.*, ${accountBalanceExpression('a')} AS balance_cents
     FROM accounts a
     WHERE a.household_id = ? ${includeArchived ? '' : 'AND a.is_archived = 0'}
     ORDER BY a.is_archived, a.sort_order, a.created_at`, context.householdId);
}
async function getCategories(env, context, includeArchived = false) {
...
async function handleBootstrap(env, context, url) {
...
async function listTransactions(env, context, url) {
    ...
    const count = await queryFirst(env.DB, `SELECT COUNT(*) AS count FROM transactions t WHERE ${clauses.join(' AND ')}`, ...params);
    return ok({ items: rows, total: count?.count ?? rows.length, limit, offset });
}
function parseTransactionBody(body) {
    const type = assertEnum(body.type, '类型', ['expense', 'income', 'transfer']);
    ...
    if (type === 'transfer' && targetAccountId === accountId)
        throw new HttpError(400, 'SAME_ACCOUNT_TRANSFER', '转出和转入账户不能相同');
    return { type, amountCents, accountId, targetAccountId, categoryId, occurredAt, merchant, note };
}
async function validateTransactionRelations(env, context, data) {
    if (!(await accountExists(env, context.householdId, data.accountId)))
        throw new HttpError(400, 'ACCOUNT_NOT_FOUND', '所选账户不存在或已归档');
    if (data.targetAccountId && !(await accountExists(env, context.householdId, data.targetAccountId)))
        throw new HttpError(400, 'TARGET_ACCOUNT_NOT_FOUND', '转入账户不存在或已归档');
    if (data.categoryId && !(await categoryExists(env, context.householdId, data.categoryId, data.type)))
        throw new HttpError(400, 'CATEGORY_NOT_FOUND', '所选分类不存在或已归档');
}
async function createTransaction(request, env, context) {
    const body = await readJson(request);
    const data = parseTransactionBody(body);
    await validateTransactionRelations(env, context, data);
    const id = crypto.randomUUID();
    await run(env.DB, `INSERT INTO transactions
     ...
     VALUES (?, ?, ?, ?, 'CNY', ?, ?, ?, ?, ?, ?, ?, ?)`, id, context.householdId, data.type, data.amountCents, data.accountId, data.targetAccountId, data.categoryId, data.merchant, data.note, data.occurredAt, context.userId, context.userId);
...
async function getTransaction(env, context, id) {
    const item = await queryFirst(env.DB, `SELECT t.*, c.name AS category_name, a.name AS account_name, ta.name AS target_account_name, u.display_name AS creator_name
     ...
     LEFT JOIN accounts ta ON ta.id = t.target_account_id
     LEFT JOIN users u ON u.id = t.created_by
     WHERE t.id = ? AND t.household_id = ?`, id, context.householdId);
    if (!item)
        throw new HttpError(404, 'TRANSACTION_NOT_FOUND', '没有找到这笔记录');
    return item;
}
async function updateTransaction(request, env, context, id) {
    const before = await getTransaction(env, context, id);
    if (before.deleted_at)
        throw new HttpError(409, 'TRANSACTION_DELETED', '这笔记录已经删除');
    const body = await readJson(request);
    const data = parseTransactionBody(body);
    const version = assertInteger(body.version, '版本', 1);
    await validateTransactionRelations(env, context, data);
    const result = await run(env.DB, `UPDATE transactions SET type = ?, amount_cents = ?, account_id = ?, target_account_id = ?, category_id = ?,
       merchant = ?, note = ?, occurred_at = ?, updated_by = ?, version = version + 1, updated_at = CURRENT_TIMESTAMP
     WHERE id = ? AND household_id = ? AND version = ? AND deleted_at IS NULL`, data.type, data.amountCents, data.accountId, data.targetAccountId, data.categoryId, data.merchant, data.note, data.occurredAt, context.userId, id, context.householdId, version);
    if ((result.meta?.changes ?? 0) === 0)
        throw new HttpError(409, 'VERSION_CONFLICT', '这笔记录刚刚被另一台设备修改，请刷新后再试');
    const after = await getTransaction(env, context, id);
    await audit(env, context, 'update', 'transaction', id, before, after);
    return ok(after);
}
async function deleteTransaction(env, context, id) {
    const before = await getTransaction(env, context, id);
    if (!before.deleted_at) {
...
async function restoreTransaction(env, context, id) {
    const before = await getTransaction(env, context, id);
    if (before.deleted_at) {
...
async function handleAccounts(request, env, context, id) {
    ...
    const before = await queryFirst(env.DB, 'SELECT * FROM accounts WHERE id = ? AND household_id = ?', id, context.householdId);
    if (!before)
        throw new HttpError(404, 'ACCOUNT_NOT_FOUND', '没有找到这个账户');
...
async function handleCategories(request, env, context, id) {
    if (request.method === 'GET' && !id)
        return ok(await getCategories(env, context, new URL(request.url).searchParams.get('includeArchived') === 'true'));
    if (request.method === 'POST' && !id) {
        const body = await readJson(request);
    ...
    if (!id)
        throw new HttpError(405, 'METHOD_NOT_ALLOWED', '不支持这个操作');
    const before = await queryFirst(env.DB, 'SELECT * FROM categories WHERE id = ? AND household_id = ?', id, context.householdId);
    if (!before)
        throw new HttpError(404, 'CATEGORY_NOT_FOUND', '没有找到这个分类');
    if (request.method === 'PATCH') {
        const body = await readJson(request);
...
async function handleBudgets(request, env, context, id) {
    ...
    if (request.method === 'POST' && !id) {
        ...
        if (categoryId && !(await categoryExists(env, context.householdId, categoryId, 'expense')))
            throw new HttpError(400, 'CATEGORY_NOT_FOUND', '所选支出分类不存在');
        const existing = await queryFirst(env.DB, `SELECT id FROM budgets WHERE household_id = ? AND period = ? AND ${categoryId ? 'category_id = ?' : 'category_id IS NULL'}`, ...(categoryId ? [context.householdId, period, categoryId] : [context.householdId, period]));
    ...
    if (request.method === 'DELETE') {
        ...
        if (!before)
            throw new HttpError(404, 'BUDGET_NOT_FOUND', '没有找到这项预算');
...
async function overviewStats(env, context, url) {
    const month = parseMonth(url.searchParams.get('month'));
    const range = monthRange(month);
    const previous = previousMonth(month);
    const previousRange = monthRange(previous);
    const sums = await queryFirst(env.DB, `SELECT
    ...
    const previousSums = await queryFirst(env.DB, `SELECT
    COALESCE(SUM(CASE WHEN type = 'income' THEN amount_cents ELSE 0 END), 0) AS income_cents,
    COALESCE(SUM(CASE WHEN type = 'expense' THEN amount_cents ELSE 0 END), 0) AS expense_cents
    ...
    const totalBalanceCents = accounts.reduce((sum, account) => sum + Number(account.balance_cents || 0), 0);
    const totalBudget = await queryFirst(env.DB, `SELECT COALESCE(SUM(amount_cents), 0) AS amount_cents FROM budgets WHERE household_id = ? AND period = ? AND category_id IS NULL`, context.householdId, month);
    const categoryBudget = await queryFirst(env.DB, `SELECT COALESCE(SUM(amount_cents), 0) AS amount_cents FROM budgets WHERE household_id = ? AND period = ? AND category_id IS NOT NULL`, context.householdId, month);
...
async function trendStats(env, context, url) {
    const month = parseMonth(url.searchParams.get('month'));
    const range = monthRange(month);
    const rows = await queryAll(env.DB, `SELECT substr(occurred_at, 1, 10) AS date,
      COALESCE(SUM(CASE WHEN type = 'income' THEN amount_cents ELSE 0 END), 0) AS income_cents,
      COALESCE(SUM(CASE WHEN type = 'expense' THEN amount_cents ELSE 0 END), 0) AS expense_cents
...
async function categoryBreakdown(env, context, url) {
    const month = parseMonth(url.searchParams.get('month'));
    const range = monthRange(month);
...
async function monthComparison(env, context, url) {
    ...
    const rows = await queryAll(env.DB, `SELECT substr(occurred_at, 1, 7) AS month,
      COALESCE(SUM(CASE WHEN type = 'income' THEN amount_cents ELSE 0 END), 0) AS income_cents,
      COALESCE(SUM(CASE WHEN type = 'expense' THEN amount_cents ELSE 0 END), 0) AS expense_cents
...
async function budgetProgress(env, context, url) {
    const month = parseMonth(url.searchParams.get('month'));
    const range = monthRange(month);
...
async function exportData(env, context, format) {
    const rows = await queryAll(env.DB, `SELECT t.occurred_at, t.type, t.amount_cents, t.currency, c.name AS category,
...
async function routeApi(request, env) {
    ...
    if (url.pathname === '/api/bootstrap' && request.method === 'GET')
        return handleBootstrap(env, context, url);
    if (url.pathname === '/api/transactions' && request.method === 'GET')
        return listTransactions(env, context, url);
    if (url.pathname === '/api/transactions' && request.method === 'POST')
        return createTransaction(request, env, context);
    const transactionMatch = url.pathname.match(/^\/api\/transactions\/([^/]+)(?:\/(restore))?$/);
    if (transactionMatch) {
        const id = decodeURIComponent(transactionMatch[1]);
        if (transactionMatch[2] === 'restore' && request.method === 'POST')
            return restoreTransaction(env, context, id);
        if (request.method === 'GET')
            return ok(await getTransaction(env, context, id));
        if (request.method === 'PATCH')
    ...
    if (accountMatch)
        return handleAccounts(request, env, context, accountMatch[1] ? decodeURIComponent(accountMatch[1]) : undefined);
    const categoryMatch = url.pathname.match(/^\/api\/categories(?:\/([^/]+))?$/);
    if (categoryMatch)
        return handleCategories(request, env, context, categoryMatch[1] ? decodeURIComponent(categoryMatch[1]) : undefined);
    const budgetMatch = url.pathname.match(/^\/api\/budgets(?:\/([^/]+))?$/);
    if (budgetMatch)
        return handleBudgets(request, env, context, budgetMatch[1] ? decodeURIComponent(budgetMatch[1]) : undefined);
    if (url.pathname === '/api/stats/overview' && request.method === 'GET')
        return overviewStats(env, context, url);
    if (url.pathname === '/api/stats/trend' && request.method === 'GET')
        return trendStats(env, context, url);
    if (url.pathname === '/api/stats/category-breakdown' && request.method === 'GET')
        return categoryBreakdown(env, context, url);
    if (url.pathname === '/api/stats/month-comparison' && request.method === 'GET')
        return monthComparison(env, context, url);
    if (url.pathname === '/api/stats/budget-progress' && request.method === 'GET')
        return budgetProgress(env, context, url);
    if (url.pathname === '/api/export/csv' && request.method === 'GET')
        return exportData(env, context, 'csv');
    if (url.pathname === '/api/export/json' && request.method === 'GET')
        return exportData(env, context, 'json');
