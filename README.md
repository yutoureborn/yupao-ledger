...
芋炮小账本是一套面向两名固定家庭成员的动态在线记账 PWA。项目使用 Cloudflare Worker、Static Assets 和 D1，在免费 `workers.dev` 地址上运行。版本 0.2.8 使用**项目内双账号认证**，不再依赖 Cloudflare Zero Trust 或 Cloudflare Access。
...
- 账户余额、分类、预算和可视化图表
- 手机、平板和桌面响应式布局
- 芋泥紫芋头与绿黄黑炮台内联 SVG、轻量组件化界面和克制页面反馈
- PWA 安装与离线应用外壳
- CSV、JSON 导出
...
- 页面减少贴纸、花边与低对比装饰，优先保证信息阅读。
- 芋头与炮台由前端内联 SVG 绘制，不再调用截图式角色图片。
- 炮台固定使用绿色、黄色和黑色组合。
- 首页 Hero、概览卡、导航、按钮、交易列表、图表、预算和发票统一设计 Token。
- 本版本不修改数据库结构。
